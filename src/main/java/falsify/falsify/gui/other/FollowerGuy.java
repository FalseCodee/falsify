...
public class FollowerGuy extends Clickable {
    ...
    public void update(int mouseX, int mouseY, double delta, ArrayList<FollowerGuy> guys) {
        ...
        for(FollowerGuy guy : guys) {
            ...
            reverseGravity(new Vec3d(guy.getX(), guy.getY(), 0),1000, delta);
        }

        reverseGravity(new Vec3d(mouseX, mouseY, 0),5000, delta);
        reverseGravity(new Vec3d(0, y, 0),500, delta);
        reverseGravity(new Vec3d(Falsify.mc.currentScreen.width, y, 0),500, delta);
        reverseGravity(new Vec3d(x, 0, 0),500, delta);
        reverseGravity(new Vec3d(x, Falsify.mc.currentScreen.height-85, 0),500, delta);
        reverseGravity(new Vec3d(Falsify.mc.currentScreen.width/2, Falsify.mc.currentScreen.height/2, 0),10000, delta);


        x += inertia.getX();
        y += inertia.getY();
        inertia = inertia.multiply(0.975);
    }

    public void doGravity(Vec3d pos, double mass, double delta) {
        Vec3d dir = pos.subtract(new Vec3d(x, y, 0));
        double distance = dir.length();
        double forceMag = (0.06674*mass)/((distance*distance)+1)*delta;
        addInertia(dir.normalize().multiply(forceMag));
    }

    public void reverseGravity(Vec3d pos, double mass, double delta) {
        Vec3d dir = new Vec3d(x, y, 0).subtract(pos);
        double distance = dir.length();
        double forceMag = (0.06674*mass)/((distance*distance)+1)*delta;
        addInertia(dir.normalize().multiply(forceMag));
    }
    ...
    public void addInertia(Vec3d inertia) {
        this.inertia = this.inertia.add(inertia);
    }

    @Override
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        drawSmoothRect(Color.WHITE, matrices, (float) x, (float) y, (float) (x+width), (float) (y+height), (float) width/2.0f, new int[] {5, 5, 5, 5});
    }
}
