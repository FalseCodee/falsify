...
class PlayersOnlineRenderModule extends RenderModule<PlayersOnlineModule> {
    ...
    public void renderModule(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredText(matrices, Falsify.mc.textRenderer, (Falsify.mc.getNetworkHandler() == null) ? "Unknown" : Falsify.mc.getNetworkHandler().getPlayerList().size() + " Players", (int) getX() + (int) width/2, (int) getY() + (int) height/2 - Falsify.mc.textRenderer.fontHeight/2, module.getTextColor().getRGB());
    }
}
