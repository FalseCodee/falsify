...
public class Nametags extends Module {
    ...
    private final ColorSetting backgroundColor = new ColorSetting("Background", new Color(0,0,0, 110));
    private final ColorSetting textColor = new ColorSetting("Background", new Color(255, 255, 255, 255));
    public Nametags() {
    ...
    public void onEvent(Event<?> event) {
        if(event instanceof EventEntityRender.Label) event.setCancelled(true);
        ...
        entityList.forEach(entity -> {
            renderLabel((LivingEntity) entity, eventRender.getDrawContext(), eventRender.getTickDelta(), background.getValue(), health.getValue(), distance.getValue());
        });
    ...
    private void renderLabel(LivingEntity entity, DrawContext context, float tickDelta, boolean drawBackground, boolean drawHealth, boolean drawDistance) {
        ...
        if(drawHealth) {
            name += " §a" + (int) entity.getHealth()+ "/" + (int) entity.getMaxHealth() + " ";
        }

        if(drawDistance) {
            name += " §c" + (int) mc.player.distanceTo(entity) + "m ";
        }
        if(drawBackground) {
            float halfWidth = fr.getStringWidth(name) / 2f;

            RenderHelper.drawSmoothRect(backgroundColor.getValue(), matrices, -halfWidth-2, -2, halfWidth+2, 13, 5, new int[]{5,5,5,5});
        }

        fr.drawCenteredString(context, name, 0, 0, textColor.getValue(), true);
        matrices.pop();
    }
}
