...
class TitleRenderModule extends RenderModule<TitleModule> {
    ...
    public void renderModule(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredTextWithShadow(matrices, Falsify.mc.textRenderer, "Legacy Client", (int) getX() + (int) width/2, (int) getY() + (int) height/2 - Falsify.mc.textRenderer.fontHeight/2, module.getTextColor().getRGB());
    }
}
