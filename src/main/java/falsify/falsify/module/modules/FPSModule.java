...
class FPSRenderModule extends RenderModule<FPSModule> {
    ...
    public void renderModule(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredText(matrices, Falsify.mc.textRenderer, "FPS: " + ((MixinMinecraft)Falsify.mc).getCurrentFps(), (int) getX() + (int) width/2, (int) getY() + (int) height/2 - Falsify.mc.textRenderer.fontHeight/2, module.getTextColor().getRGB());
    }
}
